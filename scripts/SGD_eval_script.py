...
def get_dataset_as_dict(file_path_patterns):
  ...
  return dataset_dict
def convert_pred_to_dataset_hyp(predictions, dataset_ref):
  
  dataset_hyp = deepcopy(dataset_ref)
  
  for dial_id, dial_hyp in dataset_hyp.items():
    prediction = predictions[dial_id]
    
    for turn_id,turn in prediction["turns"].items():
      assert dial_hyp["turns"][int(turn_id)]["speaker"] == "USER", "speaker turn should not have prediction."
      
      frames = dial_hyp["turns"][int(turn_id)]["frames"]
      # Empty slots and values of all frames
      frame_dict = {}
      for frame in frames:
        frame_dict[frame["service"]] = frame
        frame["state"]["slot_values"] = collections.defaultdict(lambda:[])
        
      
      for belief in turn["pred_belief"]:
        domain , slot= belief.split("-")[:2]
        value = "-".join(belief.split("-")[2:])
        frame_dict[domain]["state"]["slot_values"][slot].append(value)
...
def main(_):
  ...
  # HACK Add val set to in domain services.
  eval_path = os.path.join(FLAGS.dstc8_data_dir, FLAGS.eval_set, "schema.json")
  train_path = os.path.join(FLAGS.dstc8_data_dir, "train", "schema.json")
  dev_path = os.path.join(FLAGS.dstc8_data_dir, "dev", "schema.json")
  seen_services = get_service_set(train_path) | get_service_set(dev_path)
  in_domain_services = seen_services & get_service_set(eval_path)
  ...
  dataset_hyp = get_dataset_as_dict(
      os.path.join(FLAGS.prediction_dir, "results/zeroshot_prediction.json"))
  ...
  dataset_hyp = convert_pred_to_dataset_hyp(dataset_hyp, dataset_ref)
